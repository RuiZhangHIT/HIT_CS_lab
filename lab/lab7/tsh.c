...
int builtin_cmd(char **argv)
{
    if(!strcmp(argv[0],"quit"))     /*如果是quit，直接退出*/
    {
        exit(0);
    }
    ...
    if(!strcmp(argv[0],"jobs"))     /*如果是jobs，调用listjobs显示各个job的状态*/
    {
        listjobs(jobs);
        return 1;
    }
    if(!strcmp(argv[0],"bg") || !strcmp(argv[0],"fg"))   /*如果fg或者bg，则调用do_bgfg进行处理即可*/
    {
        do_bgfg(argv);
        return 1;
    }
...
void sigchld_handler(int sig)
{
    int olderrno = errno,status;    /* 保存errno*/
    ...
    while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)    /*回收僵死进程,waitpid返回值为正，说明有被终止或停止的子进程*/
    {
        if (sigprocmask(SIG_BLOCK, &mask, &prev_mask) < 0)
            unix_error("sigpromask error");
        job = getjobpid(jobs, pid);
        if(WIFSTOPPED(status))  /*子进程停止引起waitpid函数返回*/
        {
            job->state = ST;    /*将状态更改为ST*/
            printf("Job [%d] (%d) stopped by signal %d\n", job->jid, job->pid, WSTOPSIG(status));
        }
        else if(WIFEXITED(status))    /*子进程正常终止引起waitpid函数返回*/
        {
            deletejob(jobs, pid);   /*回收已终止的进程*/
        }
        else    /*子进程因为未捕获的信号而终止引起waitpid函数返回*/
        {
...
void sigint_handler(int sig)
{
	...
	if(sigfillset(&mask) < 0)
        unix_error("sigfillset error");
	if (sigprocmask(SIG_BLOCK, &mask, &prev_mask) < 0)  /*需要访问全局变量jobs，先暂时阻塞信号*/
            unix_error("sigpromask error");
